...
class Pix2Seq(nn.Module):
    def __init__(self,
        ...
        self.NowFuseOthers = nn.Sequential(
            nn.BatchNorm2d(self.x_inp_dim +1),
            nn.Conv2d(self.x_inp_dim +1, self.x_inp_dim * 4, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(),
            nn.Conv2d(self.x_inp_dim * 4, self.x_inp_dim, kernel_size=3, stride=1, padding=1)
        )

        # self.EmbeddingValue = nn.Embedding(2, 512)
    ...
    def IntegratePast(self, bottom_fea, prev_mask):
        '''

        :param bottom_fea: [b,64,h/4,w/4]
        :param prev_mask: [b,t,1,h/4,w/4]
        :return:
        '''
        # loop for concating all past masks
        # every time concat only one since the num of past frame is agnostic
        for ii in range(prev_mask.shape[1]):
            past_ii = prev_mask[:, ii]
            cat_past = torch.cat([bottom_fea, past_ii], dim=1)
            cat_past = self.NowFuseOthers(cat_past)
        return cat_past

    def forward(self, frames, labels=None, is_train=True):
        if not is_train:
            return self.ddim_sample_pure(frames)
        ...
        loss = None
        masks = None
        # the masks will not upsample for convenience
        for tt in range(num_frame):
            ...
            if tt == 0:
                ...
                # CatPrev = self.IntegratePast(Q4[:, tt], masks) no previous in 1st frames
                ...
                mask_cur = F.sigmoid(logits)  #
                # loss compute
                valid_mask = ((labels[:, tt, 0] >= 0)).float()  # keep the mask be valid
                loss_cur_bce = self.bce_fct(upsampled_logits[:, 0], labels[:, tt, 0].float())
                loss_cur_bce = (loss_cur_bce * valid_mask).mean()
                loss_cur_hinge = lovasz_hinge(upsampled_logits[:, 0], labels[:, tt, 0].float())
                cur_loss = loss_cur_bce + loss_cur_hinge
                loss = cur_loss
                ...
                mix_past = self.IntegratePast(Q4[:, tt], masks)
                mix_bottom = torch.cat([mix_past, noised_label], dim=1)
    ...
    @torch.no_grad()
    def ddim_sample_one_pure(self, all_hidden_states):
        ## diffusion part
        ...
        pred_down = torch.sigmoid(logits)  # [b,1,h,w]
        pred_down = (pred_down - pred_down.min()) / (pred_down.max() - pred_down.min() + 1e-8)

        return pred,pred_down

    @torch.no_grad()
    def ddim_sample_pure(self, frames): 
        assert len(frames.size()) == 5  # must be (b,t,c,h,w)
        ...
        for tt in range(num_frame):
            if tt == 0:
                ...
                cur_mask,cur_mask_down = self.ddim_sample_one_pure(fea_gather)
                masks = cur_mask.unsqueeze(1)
                masks_down = cur_mask_down.unsqueeze(1)
            else:
                mix_past = self.IntegratePast(Q4[:, tt], masks_down)

                fea_gather = (mix_past, Q8[:, tt], Q16[:, tt], Q32[:, tt])
                cur_mask,cur_mask_down = self.ddim_sample_one_pure(fea_gather)
                masks = torch.cat([masks, cur_mask.unsqueeze(1)], dim=1)
                masks_down = torch.cat([masks_down, cur_mask_down.unsqueeze(1)], dim=1)
